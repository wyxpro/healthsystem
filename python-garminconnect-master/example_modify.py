...
# today = datetime.date(2025, 3, 7)
# startdate = datetime.date(2025, 4, 3)  # Select past week
# start = 0
# limit = 100
...
#     "6": f"Get body composition data for from '{today.isoformat()}' to '{startdate.isoformat()}' (to be compatible with garminconnect-ha)",
...
#     "-": f"Get daily step data for '{today.isoformat()}' to '{startdate.isoformat()}'",
#     "/": f"Get body battery data for '{today.isoformat()}' to '{startdate.isoformat()}'",
#     "!": f"Get floors data for '{startdate.isoformat()}'",
#     "?": f"Get blood pressure data for '{today.isoformat()}' to '{startdate.isoformat()}'",
...
#     "z": f"Get progress summary from '{today.isoformat()}' to '{startdate.isoformat()}' for all metrics",
#     "A": "Get gear, the defaults, activity types and statistics",
#     "B": f"Get weight-ins from '{today.isoformat()}' to '{startdate.isoformat()}'",
...
#     "F": f"Get virtual challenges/expeditions from '{today.isoformat()}' to '{startdate.isoformat()}'",
#     "G": f"Get hill score data from '{today.isoformat()}' to '{startdate.isoformat()}'",
#     "H": f"Get endurance score data from '{today.isoformat()}' to '{startdate.isoformat()}'",
...
#                 display_json(
#                     f"api.get_body_composition('{today.isoformat()}, {startdate.isoformat()}')",
#                     api.get_body_composition(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "7":
...
#                 display_json(
#                     f"api.get_body_battery('{today.isoformat()}, {startdate.isoformat()}')",
#                     api.get_body_battery(today.isoformat(), startdate.isoformat()),
#                 )
#                 # Get daily body battery event data for 'YYYY-MM-DD'
#                 display_json(
#                     f"api.get_body_battery_events('{today.isoformat()}, {startdate.isoformat()}')",
#                     api.get_body_battery_events(startdate.isoformat()),
#                 )
#             elif i == "?":
#                 # Get daily blood pressure data for 'YYYY-MM-DD' to 'YYYY-MM-DD'
#                 display_json(
#                     f"api.get_blood_pressure('{today.isoformat()}, {startdate.isoformat()}')",
#                     api.get_blood_pressure(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "-":
#                 # Get daily step data for 'YYYY-MM-DD'
#                 display_json(
#                     f"api.get_daily_steps('{today.isoformat()}, {startdate.isoformat()}')",
#                     api.get_daily_steps(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "!":
...
#                 # Get weigh-ins data
#                 display_json(
#                     f"api.get_weigh_ins({today.isoformat()}, {startdate.isoformat()})",
#                     api.get_weigh_ins(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "C":
...
#                 display_json(
#                     f"api.get_inprogress_virtual_challenges({today.isoformat()}, {startdate.isoformat()})",
...
#                 # Get hill score data
#                 display_json(
#                     f"api.get_hill_score({today.isoformat()}, {startdate.isoformat()})",
#                     api.get_hill_score(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "H":
#                 # Get endurance score data
#                 display_json(
#                     f"api.get_endurance_score({today.isoformat()}, {startdate.isoformat()})",
#                     api.get_endurance_score(today.isoformat(), startdate.isoformat()),
#                 )
#             elif i == "I":
...
menu_options = {
    "1": "Get full name",
    "2": "Get unit system",
    "4": f"Get activity data for '{startdate.isoformat()}' (compatible with garminconnect-ha)",#每日活动
    "7": f"Get stats and body composition data for '{startdate.isoformat()}'",#每日状态，包括各项数据
    "8": f"Get steps data for '{startdate.isoformat()}'",#每15分钟
    "9": f"Get heart rate data for '{startdate.isoformat()}'",#实时
    ...
    "a": f"Get resting heart rate data for {startdate.isoformat()}'",#静息心率每日一次
    "c": f"Get sleep data for '{startdate.isoformat()}'",#实时
    "d": f"Get stress data for '{startdate.isoformat()}'",
    "e": f"Get respiration data for '{startdate.isoformat()}'",#实时
    "f": f"Get SpO2 data for '{startdate.isoformat()}'",#每小时
    "x": f"Get Heart Rate Variability data (HRV) for '{startdate.isoformat()}'",#每日
    "U": f"Get Fitness Age data for {startdate.isoformat()}",#每日
    "Z": "Remove stored login tokens (logout)",
    "q": "Exit",
}
    ...
    if api:
        try:
            ...
            if i == "1":
                ...
                # Get stats and body composition data for 'YYYY-MM-DD'
                display_json(
                    f"api.get_stats_and_body('{startdate.isoformat()}')",
                    api.get_stats_and_body(startdate.isoformat()),
                )
            # USER STATISTICS LOGGED
            elif i == "8":
                # Get steps data for 'YYYY-MM-DD'
                ...
                # Get daily body battery data for 'YYYY-MM-DD' to 'YYYY-MM-DD'
                display_json(
                    f"api.get_body_battery('{startdate.isoformat()}', '{enddate.isoformat()}')",
                    api.get_body_battery(startdate.isoformat(), enddate.isoformat()),
                )
                # Get daily body battery event data for 'YYYY-MM-DD'
                display_json(
                    f"api.get_body_battery_events('{startdate.isoformat()}', '{enddate.isoformat()}')",
                    api.get_body_battery_events(enddate.isoformat()),
                )
            elif i == "-":
                # Get daily step data for 'YYYY-MM-DD'
                display_json(
                ...
                # Get resting heart rate data for 'YYYY-MM-DD'
                display_json(
                    f"api.get_rhr_day('{startdate.isoformat()}')",
                    api.get_rhr_day(startdate.isoformat()),
                )
            elif i == "c":
                # Get sleep data for 'YYYY-MM-DD'
                display_json(
                ...
                # Get fitness age data
                display_json(
                    f"api.get_fitnessage_data({startdate.isoformat()})",
                    api.get_fitnessage_data(startdate.isoformat()),
                )
            elif i == "Z":
                # Remove stored login tokens for Garmin Connect portal
                tokendir = os.path.expanduser(tokenstore)
