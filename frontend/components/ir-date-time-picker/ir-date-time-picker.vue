<template>
	<view>
		<view :class="['date', type]" :style="{borderColor, opacity: disabled ? 0.6 : 1, height}" @tap="tapInput">
			...
			<slot name="arrow">
				<view v-if="showArrow && (!showClear || showClear && !date)" class="arrow-icon"
			...
			<picker-view class="picker-view" :indicator-style='indicatorStyle' :mask-style='maskStyle'
				...
				<picker-view-column>
					<view class="item" v-for="(item,index) in days" :key="index"
						:style="[{color: index === pickerVal[2] ? activeColor : pickerColor}, itemStyle]">{{item}}日
					</view>
				</picker-view-column>
				<picker-view-column>
					<view class="item" v-for="(item,index) in hours" :key="index"
						:style="[{color: index === pickerVal[3] ? activeColor : pickerColor}, itemStyle]">{{item}}时
					</view>
				</picker-view-column>
				<picker-view-column>
					<view class="item" v-for="(item,index) in minutes" :key="index"
						:style="[{color: index === pickerVal[4] ? activeColor : pickerColor}, itemStyle]">{{item}}分
					</view>
				</picker-view-column>
			</picker-view>
