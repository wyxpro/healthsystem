	...
	export default {
		...
		methods: {
			...
			unCheckedOther(childInstance) {
				const values = []
				if(childInstance.name==this.mutualExclusionName){
					if(childInstance.isChecked){
						this.children.map(child => {
							// 将被选中的checkbox，放到数组中返回
							if (child.name==this.mutualExclusionName) {
					...
					this.children.map(child => {
						// 将被选中的checkbox，放到数组中返回
						if (child.isChecked && child.name!=this.mutualExclusionName) {
							values.push(child.name)
						}
					})
				}
