		...
		<div class="form-box">
			<div v-if="currentPage === 1">
				...
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					<p class="title title0">一般情况调查表</p>
			...
			<div v-if="currentPage === 2">
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					<p class="title title0">一般情况调查表</p>
					<u-form-item label="3、出生年月" prop="birthDate" class="birthDate" required>
						<xp-picker mode="ym" :yearRange=[1900,null] v-model="BasicInfo.birthDate" />
					</u-form-item>
			...
			<div v-if="currentPage === 3">
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					...
					<u-form-item label="2、电话" prop="phone" class="phone" required>
						<u--input v-model="BasicInfo.phone" style="background-color: white;"></u--input>
					</u-form-item>
					<button class="button" @click="prevPage">上一题</button>
					<button class="button" @click="nextPage">下一题</button>
				</u-form>
			...
			<div v-if="currentPage === 9">
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					...
					<u-form-item label="10、如果有，是在什么时候？" v-show="BasicInfo.trauma!='无'" prop="traumaTime"
			...
			<div v-if="currentPage === 17">
				<<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo"
					...
					<u-form-item label="5、过去，在您遇到急难情况时，曾经得到的经济支持和解决实际问题的帮助来源有：" prop="favour" class="favour" required>
						<u-radio-group placement="column" v-model="BasicInfo.favour">
							<u-radio v-for="item in favourList" :name="item" :label="item" :key="item"
								style="margin: 5rpx;"></u-radio>
						</u-radio-group>
					</u-form-item>
			...
			<div v-if="currentPage === 18">
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					...
					<u-form-item label="6、过去，在您遇到急难情况时，曾得到的安慰和关心来源有：" prop="comfort" class="comfort" required>
						<u-radio-group placement="column" v-model="BasicInfo.comfort">
							<u-radio v-for="item in comfortList" :name="item" :label="item" :key="item"
								style="margin: 5rpx;"></u-radio>
						</u-radio-group>
					</u-form-item>
					<u-form-item v-show="BasicInfo.favour=='其他（请列出）'" prop="comfort_other" class="comfort_other">
				...
				<u-form class="form" labelPosition="top" labelWidth='100%' ref="form" :model="BasicInfo" :rules="rules">
					...
					<u-form-item label="4、家庭经济困难" prop="familyFinanceDif_happen" class="familyFinanceDif_happen"
						...
						<u-radio-group placement="column" v-model="BasicInfo.familyFinanceDif_happen">
							<u-radio v-for="item in lesTypeList" :name="item" :label="item" :key="item"
								style="margin: 5rpx;"></u-radio>
						</u-radio-group>
					</u-form-item>
					<u-form-item
	...
	export default {
		data() {
			return {
				...
				labelWidth: '80%',
				sexList: ["男", "女"],
				cultureList: ["本科", "硕士研究生", "博士研究生"],
				isSingleParentList: ["是", "否"],
				isAbandonList: ["是", "否"],
				residentialList: ["独居", "集体宿舍"],
				...
				favourList: ["无任何来源", "配偶", "其他家人", "朋友", "亲戚", "同事", "工作单位", "党团公会等官方或半官方组织", "宗教、社会团体等非官方组织", "其他（请列出）"],
...
<style lang="scss" scoped>
	.title {
		...
		margin: 10px auto;
		border-left: 10px solid rgb(64, 158, 255);
	}
