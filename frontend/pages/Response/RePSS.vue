...
<script>
	...
	export default {
		...
		methods: {
			getData() {
				...
				http.get("/scale/ps/getlist?currentPage=" + this.currentPage + "&pageSize=" + this.pageSize).then(
					response => {
						...
						// 一共5颗星星
						if (this.anxietyScore >= 0 && this.anxietyScore <= 19) {
							this.anxietyRating = 1;
						} else if (this.anxietyScore <= 39) {
							this.anxietyRating = 3;
						} else if (this.anxietyScore <= 56) {
			...
			getResult() {
				const context =
					"不过不要惊慌，此结果仅反映您最近一个月的压力状态，可能是当前工作生活状态变化的正常反应，不代表焦虑障碍。建议您积极调节情绪并定期测评，一般随着时间的推移会逐渐改善。如果长期持续此状态，"
				const adviceArray = [
					"最好咨询心理医生或心理医学工作者。",
					"建议咨询心理医生或精神科医生。",
					"一定要看心理医生或精神科医生。"
...
<style lang="scss" scoped>
	...
	.box-card {
		...
		height: 190px;
		background: #eaeaea;
		// background-image: url('@/static/images/scale/live.jpg');
