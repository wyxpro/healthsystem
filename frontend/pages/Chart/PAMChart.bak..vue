...
<template>
	<view>
		...
		<div class="ar-wrapper">
			<!--  音视频情感推断 -->
			<l-echart class="emotion" ref="emotion" @finished="initDrawEmotion"></l-echart>
		</div>

		<p class="note">
			<!--      注：<b>情感来自于 EMA、音频、视频</b>-->
			<br>
			<b ref="saying">元气满满</b>
			<br>
			歌曲推荐：<a ref="song_recom" href="" target="_Blank"></a>
		...
		methods: {
			...
			getData() {
				...
				http.post("/emaanswer/getPamListByDate", {
					...
					this.recommendSong(response.result.countNegtive, response.result.countPositive)
			...
			recommendSong(nCount, pCount) {
				if (nCount >= pCount) { // 负向情绪 >= 正向情绪
					this.$refs.saying.innerHTML = "赶快让自己乐观豁达起来吧，只要你的心理一改变，身体精神也会随之变化。" +
						"那样你会得到一种永远向上的力量，这种力量会使你快乐而充满活力!"
					console.log("dsdsdsdsdsdfg")
				} else {
					this.$refs.saying.innerHTML = "今天的心情不错呀，保持愉悦，开启元气满满的一天！为你加油打call！"
					console.log("aaaaaaaaaaaaaaa")
				}

				var songName = new Array()
				songName[0] = "Good Life"
				songName[1] = "阳光路上"
				songName[2] = "旗开得胜"
				songName[3] = "爱拼才会赢"
				songName[4] = "夜空中最亮的星"
				...
				songUrl[0] = "https://music.163.com/#/song?id=466122659"
				songUrl[1] = "https://music.163.com/#/song?id=488641056"
				songUrl[2] = "https://music.163.com/#/song?id=5240551"
				songUrl[3] = "https://music.163.com/#/song?id=179242"
				songUrl[4] = "https://music.163.com/#/song?id=25706282"
				songUrl[5] = "https://music.163.com/#/song?id=25640392"

				let number = this.getRandomInt(0, 5);
				
				// console.log(songName)
				// console.log(songUrl)

				this.$refs.song_recom.innerHTML = songName[number]
				this.$refs.song_recom.setAttribute("href", songUrl[number])
			...
			async initDrawEmotion() {
				if (!this.dataGeted) {
					setTimeout(this.initDrawEmotion, 500);
				} else {
					// chart 图表实例不能存在data里
					chartsEmotion = await this.$refs.emotion.init(echarts);
					this.drawEmotion();
				}
			},
			/**
			...
			drawEmotion() {
				...
				chartsEmotion.setOption({
					...
					series: [{
						name: "情感",
						type: 'bar', // 图表类型，折线图还是柱状图还是饼图
						data: this.emotionList, // arousal data
						//柱子宽度
						// barWidth: "50%",
						smooth: true,
					}]
